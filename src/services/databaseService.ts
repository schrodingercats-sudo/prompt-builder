...
export class DatabaseService {
  ...
  async getPublicPrompts(limit: number = 50): Promise<SavedPrompt[]> {
    ...
    return data.map(prompt => ({
      ...
      user_display_name: (prompt as any).users.display_name
    }));
  }
      ...
      if (!promptsError && recentPrompts) {
        recentPrompts.forEach(prompt => {
          activities.push({
            ...
            userEmail: (prompt as any).users.email
          });
        });
      }
  ...
  async checkAPIStatus(): Promise<{
    ...
    type StatusEntry = { status: 'connected' | 'disconnected'; message: string };
    const result: { gemini: StatusEntry; firebase: StatusEntry; supabase: StatusEntry; razorpay: StatusEntry } = {
      gemini: { status: 'disconnected', message: 'Not checked' },
      firebase: { status: 'disconnected', message: 'Not checked' },
      supabase: { status: 'disconnected', message: 'Not checked' },
      razorpay: { status: 'disconnected', message: 'Not checked' }
    };
    ...
    try {
      ...
      if (!error) {
        result.supabase = { status: 'connected' as const, message: 'Database connection successful' };
      } else {
    ...
    try {
      ...
      if (geminiKey && geminiKey.length > 0) {
        result.gemini = { status: 'connected' as const, message: 'API key configured' };
      } else {
    ...
    try {
      ...
      result.firebase = { status: 'connected' as const, message: 'Authentication service active' };
    } catch (error) {
    ...
    try {
      ...
      if (razorpayKey && razorpayKey.length > 0) {
        result.razorpay = { status: 'connected' as const, message: 'Payment gateway configured' };
      } else {
